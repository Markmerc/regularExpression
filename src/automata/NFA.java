...
public class NFA extends AbstractNFA {
	...
	// epsilon closure - grow start state by checking if value of edge is
	// epsilon
			...
			if (c == '\\') {
				if (pos == strlen - 1) {
					throw new IllegalArgumentException(
							"a meta character must follow a \\");
				}
				pos += 1;
				c = regex.charAt(pos);
	...
	protected AbstractNFA unionOf(AbstractNFA nfa1, AbstractNFA nfa2) {
		...
		// Create new start state, and create epsilon transitions to old start
		// states
		int start = StateNumberKeeper.getNewStateNumber();
	...
	protected AbstractNFA plusOf(AbstractNFA nfa) {
		Set<Edge> newEdges = nfa.getEdges();
		Set<Integer> oldFinal = nfa.getFinalStates();
		int start = nfa.getStartStates().iterator().next();

		for (int x : oldFinal) {
			newEdges.add(new Edge(x, start, new Labels(0)));
		}

		nfa.setEdges(newEdges);
		return nfa;
	}

	@Override
	protected AbstractNFA maxOnceOf(AbstractNFA nfa) {
		AbstractNFA questionMark = new NFA();
		...
		Set<Integer> newFinal = new HashSet<Integer>(oldFinal);
		Set<Edge> newEdges = new HashSet<Edge>(oldEdges);
		int questionMarkStart = StateNumberKeeper.getNewStateNumber();

		newEdges.add(new Edge(questionMarkStart, oldStart, new Labels(0)));

		newFinal.add(questionMarkStart);

		questionMark.setStartStates(mkStates(questionMarkStart));
		questionMark.setFinalStates(newFinal);
		questionMark.setEdges(newEdges);
	...
	public static void main(String[] args) {
		try {
			if (args.length < 2) {
				System.out.println("usage: NFA arg1 arg2\n"
			...
			System.out.println("Illegal regex expression");
		}
		return;
	}
}